...
type QuickFilter = "ALL" | "OVERDUE" | "WAITING_FEEDBACK" | "IN_PROGRESS" | "COMPLETED";
...
const PROGRESS_OPTIONS = ["REQUESTED", "DEVELOPING", "SENT", "FEEDBACK", "COMPLETED"] as const;
const RESULT_OPTIONS = ["WAITING", "APPROVED", "REVISE", "REJECTED", "CONVERTED_TO_ORDER", "CLOSED_NO_ORDER"] as const;
const ALERT_OPTIONS = ["", "ON_TRACK", "DUE_SOON", "OVERDUE", "WAITING_FEEDBACK", "FOLLOW_UP_DUE", "DONE"] as const;
...
function statusBadgeClass(v: string) {
  ...
  if (s === "SENT") return "bg-indigo-50 text-indigo-700 border border-indigo-200";
  if (s === "FEEDBACK") return "bg-violet-50 text-violet-700 border border-violet-200";
  return "bg-slate-50 text-slate-700 border border-slate-200";
}
function resultBadgeClass(v: string) {
  const s = (v || "").toUpperCase();
  if (s === "CONVERTED_TO_ORDER") return "bg-emerald-100 text-emerald-700 border border-emerald-200";
  if (s === "CLOSED_NO_ORDER") return "bg-rose-100 text-rose-700 border border-rose-200";
  if (s === "APPROVED") return "bg-blue-100 text-blue-700 border border-blue-200";
  return "bg-slate-100 text-slate-700 border border-slate-200";
}
...
function normalizeForm(input: RequestForm): RequestForm {
  let resultStatus = normalizeResultStatus(input.result_status);
  let progressStatus = normalizeProgressStatus(input.progress_status, resultStatus);
  if (resultStatus === "CONVERTED_TO_ORDER" || resultStatus === "CLOSED_NO_ORDER") progressStatus = "COMPLETED";
  if (progressStatus !== "COMPLETED" && (resultStatus === "CONVERTED_TO_ORDER" || resultStatus === "CLOSED_NO_ORDER")) resultStatus = "WAITING";
  return { ...input, progress_status: progressStatus, result_status: resultStatus, is_converted_to_order: resultStatus === "CONVERTED_TO_ORDER" };
}
function computeSummary(items: any[]): Summary {
  const total_requests = items.length;
  const in_progress = items.filter((r) => normalizeProgressStatus(r.progress_status, r.result_status) !== "COMPLETED").length;
  const completed = items.filter((r) => normalizeProgressStatus(r.progress_status, r.result_status) === "COMPLETED").length;
  const converted_requests = items.filter((r) => normalizeResultStatus(r.result_status) === "CONVERTED_TO_ORDER").length;
  const overdue = items.filter((r) => String(r.alert_status || "").toUpperCase() === "OVERDUE").length;
  const waiting_feedback = items.filter((r) => ["WAITING_FEEDBACK", "FOLLOW_UP_DUE"].includes(String(r.alert_status || "").toUpperCase())).length;
  return {
    total_requests,
    in_progress,
    completed,
    converted_requests,
    conversion_pct: total_requests ? (converted_requests / total_requests) * 100 : 0,
    overdue,
    waiting_feedback,
  };
}
...
function FileUploader({ label, files, onChange }: { label: string; files: FileItem[]; onChange: (files: FileItem[]) => void; }) {
  async function upload(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (json?.ok && json?.file) onChange([...(files || []), json.file]);
      ...
      <div className="space-y-2">
        {(files || []).map((f, idx) => (
          <div key={idx} className="flex items-center justify-between rounded border px-3 py-2 text-sm">
            <a className="truncate text-blue-600 underline" href={f.url} target="_blank" rel="noreferrer">{f.name}</a>
            <Button type="button" variant="outline" size="sm" onClick={() => onChange(files.filter((_, i) => i !== idx))}>Remove</Button>
          </div>
        ))}
      </div>
    </div>
...
export default function SampleRequestsPage() {
  ...
  const [requestDateFrom, setRequestDateFrom] = React.useState("");
  const [requestDateTo, setRequestDateTo] = React.useState("");
  const [targetShipDateFrom, setTargetShipDateFrom] = React.useState("");
  const [targetShipDateTo, setTargetShipDateTo] = React.useState("");
  ...
  if (statusFilter !== "ALL") listQs.set("status", statusFilter);
  if (resultFilter !== "ALL") listQs.set("result_status", resultFilter);
  if (alertFilter !== "ALL") listQs.set("alert_status", alertFilter);
  if (quickFilter !== "ALL") listQs.set("quick", quickFilter);
  if (requestDateFrom) listQs.set("request_date_from", requestDateFrom);
  if (requestDateTo) listQs.set("request_date_to", requestDateTo);
  if (targetShipDateFrom) listQs.set("target_ship_date_from", targetShipDateFrom);
  if (targetShipDateTo) listQs.set("target_ship_date_to", targetShipDateTo);
  ...
  async function hydrateNo(buyerCode: string, requestDate: string) {
    ...
    if (nextNo) setForm((prev) => ({ ...prev, request_no: nextNo }));
  }
  React.useEffect(() => {
    if (!form.id && form.buyer_code) hydrateNo(form.buyer_code, form.request_date || todayISO());
  }, [form.buyer_code, form.request_date]);

  function setField<K extends keyof RequestForm>(key: K, value: RequestForm[K]) { setForm((prev) => ({ ...prev, [key]: value })); }
  function resetFilters() {
    ...
    setRequestDateFrom("");
    setRequestDateTo("");
    setTargetShipDateFrom("");
    setTargetShipDateTo("");
    setQuickFilter("ALL");
  }
  function onNew() { setSelectedId(""); setForm(makeEmptyForm()); }
  function load(item: any) {
    setSelectedId(item.id || "");
    const merged = normalizeForm({ ...makeEmptyForm(), ...item, attachments: item.attachments || [], reference_images: item.reference_images || [], shipment_proof_files: item.shipment_proof_files || [] });
    setForm(merged);
  }
  async function save() {
    if (isSaving) return;
    ...
    try {
      ...
      if (!payload.request_title.trim()) { alert("Request Title is required."); return; }
      if (!payload.buyer_id || !payload.buyer_code) { alert("Please select a buyer."); return; }
      if (!payload.id) {
      ...
      if (!json?.ok) {
        if (!payload.id && hasDuplicateRequestNoError(json)) {
          ...
          if (refreshedNo) setForm((prev) => ({ ...prev, request_no: refreshedNo }));
      ...
      await mutate();
      if (json?.item) load(json.item);
      else if (!payload.id) onNew();
      alert("Saved");
    } finally {
      setIsSaving(false);
    }
  ...
  const listTitle = quickFilter === "ALL" ? "List" : `List · ${quickFilter.replace(/_/g, " ")}`;
  ...
  return (
    <AppShell title="Sample Requests">
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Quick Filters</CardTitle></CardHeader>
          <CardContent className="flex flex-wrap gap-2">
            {(["ALL", "OVERDUE", "WAITING_FEEDBACK", "IN_PROGRESS", "COMPLETED"] as QuickFilter[]).map((key) => (
              <Button key={key} type="button" variant={quickFilter === key ? "default" : "outline"} onClick={() => setQuickFilter(key)}>
        ...
        <div className="grid gap-6 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <Card>
              <CardHeader><CardTitle className="text-base">{listTitle}</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="md:col-span-2">
                    <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search title / request no / buyer / details..." />
                  </div>
                  <div>
                  ...
                  <div>
                    <Select value={statusFilter} onValueChange={setStatusFilter}>
                      ...
                      <SelectContent>
                        ...
                        {PROGRESS_OPTIONS.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    <Select value={resultFilter} onValueChange={setResultFilter}>
                      ...
                      <SelectContent>
                        ...
                        {RESULT_OPTIONS.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    <Select value={alertFilter} onValueChange={setAlertFilter}>
                      <SelectTrigger><SelectValue placeholder="Alert" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="ALL">All Alerts</SelectItem>
                        {ALERT_OPTIONS.filter(Boolean).map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    <Label className="mb-1 block text-xs text-muted-foreground">Request Date From</Label>
                    <Input type="date" value={requestDateFrom} onChange={(e) => setRequestDateFrom(e.target.value)} />
                  </div>
                  <div>
                    <Label className="mb-1 block text-xs text-muted-foreground">Request Date To</Label>
                    <Input type="date" value={requestDateTo} onChange={(e) => setRequestDateTo(e.target.value)} />
                  </div>
                  <div>
                    <Label className="mb-1 block text-xs text-muted-foreground">Target Ship From</Label>
                    <Input type="date" value={targetShipDateFrom} onChange={(e) => setTargetShipDateFrom(e.target.value)} />
                  </div>
                  <div>
                    <Label className="mb-1 block text-xs text-muted-foreground">Target Ship To</Label>
                    <Input type="date" value={targetShipDateTo} onChange={(e) => setTargetShipDateTo(e.target.value)} />
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button type="button" variant="outline" onClick={() => window.open("/dashboards/samples", "_blank")}>Open Dashboard</Button>
                </div>
                <div className="space-y-3">
                  {items.length === 0 ? (
                    <div className="rounded border border-dashed px-3 py-6 text-center text-sm text-muted-foreground">No requests match the current filters.</div>
                  ) : items.map((r: any) => (
                    <div key={r.id} className={`rounded-xl border p-4 ${selectedId === r.id ? "border-blue-500 bg-blue-50/40" : ""}`}>
                      <div className="flex items-start justify-between gap-3">
                        <button type="button" className="w-full text-left" onClick={() => load(r)}>
                          ...
                          <div className="mt-1 text-xs text-muted-foreground">Request Date: {r.request_date || "—"} · Target Ship: {r.target_ship_date || "—"}</div>
                          <div className="mt-1 text-xs text-muted-foreground">Owner: {r.our_owner_name || "—"} · Days Open: {r.days_open ?? 0}</div>
                          ...
                          <div className="mt-2 flex flex-wrap gap-2 text-xs">
                            ...
                            <span className={`inline-flex rounded-full px-2 py-1 ${statusBadgeClass(r.progress_status)}`}>{normalizeProgressStatus(r.progress_status, r.result_status)}</span>
                            <span className={`inline-flex rounded-full px-2 py-1 ${resultBadgeClass(r.result_status)}`}>{normalizeResultStatus(r.result_status)}</span>
                          </div>
                        </button>
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                {buyersLoading ? null : buyersLoadError ? (
                  <div className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">Buyer list load issue: {buyersLoadError}</div>
                ) : buyers.length === 0 ? (
                  <div className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">Buyer list is empty. Check /api/sample-requests/options and your buyer master data.</div>
                ) : null}
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    <Label>Buyer</Label>
                    <Select value={form.buyer_id && buyers.some((b) => b.id === form.buyer_id) ? form.buyer_id : "__NONE__"} onValueChange={(v) => {
                      if (v === "__NONE__") {
                        setForm((prev) => ({ ...prev, buyer_id: "", buyer_name: "", buyer_code: "", request_no: "" }));
                        return;
                      }
                      const b = buyers.find((x) => x.id === v);
                      setForm((prev) => ({ ...prev, buyer_id: v, buyer_name: b?.name || "", buyer_code: b?.code || "GEN", request_no: prev.id ? prev.request_no : "" }));
                    }}>
                  ...
                  <div>
                    <Label>Status</Label>
                    <Select value={normalizeProgressStatus(form.progress_status, form.result_status)} onValueChange={(v) => {
                      const nextProgress = normalizeProgressStatus(v, form.result_status);
                      setForm((prev) => ({ ...prev, progress_status: nextProgress, result_status: nextProgress === "COMPLETED" ? prev.result_status : "WAITING" }));
                    }}>
                  ...
                  <div>
                    ...
                    <Select value={normalizeResultStatus(form.result_status)} disabled={resultDisabled} onValueChange={(v) => {
                      const nextResult = normalizeResultStatus(v);
                      setForm((prev) => ({
                        ...prev,
                        progress_status: nextResult === "CONVERTED_TO_ORDER" || nextResult === "CLOSED_NO_ORDER" ? "COMPLETED" : (prev.progress_status === "COMPLETED" && nextResult === "WAITING" ? "FEEDBACK" : prev.progress_status),
                  ...
                  <div className="md:col-span-2">
                    ...
                    <Textarea className="min-h-[140px]" value={form.requested_items_text} onChange={(e) => setField("requested_items_text", e.target.value)} placeholder="e.g. cubic earring styles, round / drop / heart options, silver + gold plating, send by 5/10" />
                  </div>
                  <div className="md:col-span-2">
                ...
                <div className="rounded border bg-slate-50 px-3 py-2 text-sm text-slate-700">
                  Status flow is REQUESTED → DEVELOPING → SENT → FEEDBACK → COMPLETED. Final outcomes can only be saved when Status is COMPLETED.
                </div>
