  ...
  async function deleteLedgerRow(row: LedgerRow) {
    ...
    if (row.ref_type) {
      if (row.source_exists && !row.source_deleted) {
        const statusText = row.source_status ? `, status: ${row.source_status}` : ''
        refLabel = `\nLinked source is still active: ${row.ref_type}${row.ref_id ? ` (${row.ref_id})` : ''}${statusText}`
      } else if (row.source_exists && row.source_deleted) {
        refLabel = `\nOriginal source tag only: ${row.ref_type}${row.ref_id ? ` (${row.ref_id})` : ''} - source is already deleted`
      } else {
        refLabel = `\nOrphaned source tag: ${row.ref_type}${row.ref_id ? ` (${row.ref_id})` : ''} - source record was not found`
      }
    }
    const confirmed = window.confirm(
      `Delete this cashbook line?\n\n${row.tx_date} | ${row.description} | ${fmtMoney(row.amount)} ${row.currency}${refLabel}`
    )
    if (!confirmed) return
    ...
    try {
      const res = await fetch(`/api/finance/cash-ledger/${row.id}`, { method: 'DELETE' })
      const json = await res.json()
      if (!res.ok || !json?.ok) throw new Error(json?.error || 'Failed to delete cashbook line')
      await refreshAll()
    } catch (e: any) {
      setMessage(e?.message || 'Failed to delete cashbook line')
    } finally {
      setDeletingRowId('')
    }
  }
      ...
      <div className="order-3 space-y-3 rounded-lg border border-gray-200 bg-white p-4">
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="min-w-full text-xs">
            ...
            <tbody>
              {loading ? (
                <tr><td className="px-2.5 py-4 text-center text-gray-500" colSpan={10}>Loading...</td></tr>
              ) : visibleRows.length === 0 ? (
                <tr><td className="px-2.5 py-4 text-center text-gray-500" colSpan={10}>No data</td></tr>
              ) : visibleRows.map((row) => (
                <tr key={row.id} className="border-b">
                  ...
                  <td className="px-2.5 py-2 text-center">
                    <button
                      ...
                      className="rounded-md border border-red-200 px-2 py-1 text-[11px] font-medium text-red-600 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                    >
