...
export async function POST(
  ...
  try {
    ...
    if (["CANCELLED", "CANCELED", "DELETED"].includes(current)) {
      return bad("Cancelled/deleted shipment cannot be confirmed", 409);
    }
    if (current === "SHIPPED") {
      return ok({ shipment: { ...shipment, status: "SHIPPED" }, already_done: true });
    }
    ...
    return ok({ shipment: updated, status_changed_to: "CONFIRMED" });
  } catch (e: any) {
    console.error("POST /api/shipments/[id]/confirm error:", e);
    return bad(e?.message || "Confirm shipment failed", 500);
  }
}
