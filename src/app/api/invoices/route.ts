...
export async function PUT(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: updatedHeaderRows, error: hUpErr } = await supabaseAdmin
      .from("invoice_headers")
      .update(headerPatch)
      .eq("id", id)
      .select("*");
    ...
    const newLines = await loadInvoiceLines(id, updatedHeaderRows?.[0]?.invoice_no ?? existing.invoice_no);
    const total = computeTotalAmount(newLines);

    const { data: finalHeaderRows, error: totalErr } = await supabaseAdmin
      .from("invoice_headers")
    ...
    const finalHeader = finalHeaderRows?.[0] ?? updatedHeaderRows?.[0] ?? null;
    ...
    return ok({
      header: finalHeader
        ? {
            ...finalHeader,
            invoice_date: normalizeDate10(finalHeader.invoice_date),
            etd: normalizeDate10(finalHeader.etd),
            eta: normalizeDate10(finalHeader.eta),
          }
        : null,
      lines: outLines,
